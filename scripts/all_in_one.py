...
def run_robofsm_graph(
    ...
    while True:
        ...
        # tensors
        s.quat_w = th.from_numpy(mj_data.sensordata[sensor_name2slice['quat']]).to(dtype=th.float32)
        s.linvel_b = th.from_numpy(mj_data.sensordata[sensor_name2slice['linvel']]).to(dtype=th.float32)
        s.angvel_b = th.from_numpy(mj_data.sensordata[sensor_name2slice['angvel']]).to(dtype=th.float32)
        s.qpos = th.from_numpy(mj_data.qpos[7:]).to(dtype=th.float32)
        s.qvel = th.from_numpy(mj_data.qvel[6:]).to(dtype=th.float32)
